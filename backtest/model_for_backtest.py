...
class Model:
    ...
    def get_data(self, period):
        ...
        self.df = yf.download(ticker, start=period["start"], end=period["end"], interval=mp.get("interval"))
    ...
    def predict_tommorow(self, lastn_df): # function to preedict future price
        ...
        lastn_days = lastn_days.reshape(1, mp.get("window_size"), len(mp.get("feature_cols")))

        predicted_price_tmmrw = self.model.predict(lastn_days)
        predicted_price_tmmrw = self.saved_scaler[mp.get("predicted_cols")].inverse_transform(predicted_price_tmmrw)
...
This class is optimized to work with the backtesting class efficiently without
wasting much time and resources.
