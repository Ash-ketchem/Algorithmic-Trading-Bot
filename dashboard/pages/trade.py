...
dash.register_page(__name__, path="/trade", title="Forward Test")


COLORS = [str(cl.YELLOW), str(cl.ORANGE), str(cl.RED), str(cl.BLUE), str(cl.PURPLE)]
...
content = [
    ...
    dbc.CardBody(
        [
            ...
            html.P(
                "Strategy Used : 3 EMA Crossover  ",
                className="card-text",
            ),
            html.P(
                "Model Used : LSTM model ",
                className="card-text",
            ),
        ]
    ),
...
layout = [
    ...
    dbc.Row(
        [
            dbc.Col(
                Box(children=dbc.Card(content, color="dark", inverse=True)),
                width=12,
                lg=3,
            ),
            dbc.Col(
                [
                    ...
                    dcc.Interval(
                        id="account-update", interval=1000 * 10, n_intervals=0
                    ),
                ],
                width=12,
                lg=8,
...
@callback(Output("account_data", "children"), [Input("account-update", "n_intervals")])
    ...
    account_content = [
        ...
        dbc.CardBody(
            [
                ...
                for col, val in account_data.items()
            ]
            if account_data
            else html.H5("Sorry No Data Available...")
        ),
    ]
